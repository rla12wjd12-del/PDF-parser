...
# ──────────────────────────────────────────────────────────────────────────────
from parsers.tech_career_table_only_core import (
    _agent_log,
...
def _table_only_postprocess_enabled() -> bool:
    """
    table-only 후처리 ON/OFF 스위치.
    - 기본: ON
    - OFF: 환경변수 PDFPARSER_TABLE_ONLY_POSTPROCESS=0/false/no/off
    """
    v = str(os.getenv("PDFPARSER_TABLE_ONLY_POSTPROCESS", "1") or "").strip().lower()
    return v not in {"0", "false", "no", "off"}


def _squash_newlines_in_obj(obj):
    """
    기술경력(표 기반) 결과에서 줄바꿈을 제거해 한 줄로 붙인다.
    - OFF 모드에서도 출력 형태가 흔들리지 않게 하기 위한 최종 정규화 단계.
    - main.py의 JSON 저장 전 스쿼시 로직과 동일한 정책(줄바꿈+주변 공백 제거 후 공백 정규화).
    """
    import re
...
def parse_page_2(ctx: DocumentContext, page_num: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not table_rows:
            ...
            try:
                _agent_log(
                    run_id="table-only",
                    hypothesis_id="T",
                    location="page_2_parser.py:parse_page_2:no_table_rows",
        ...
        if _table_only_postprocess_enabled():
            ...
            try:
                _agent_log(
                    run_id="table-only",
                    hypothesis_id="T",
                    location="page_2_parser.py:parse_page_2:postprocess_disabled",
                    message="table-only postprocess disabled by env",
                    data={"page_num_1based": page_num_1based},
                )
            except Exception:
                pass
        ...
        # 7) 최종 출력 정규화: 줄바꿈 제거(붙여쓰기)
        careers = _squash_newlines_in_obj(table_rows)
        ...
        try:
            _agent_log(
                run_id="table-only",
                hypothesis_id="T",
                location="page_2_parser.py:parse_page_2:return",
