...
def parse_education_info(page) -> List[Dict[str, Any]]:
    ...
    try:
        _DEG_STATUS = re.compile(r"^(학사|석사|박사과정|박사수료|박사|없음)\s*\[\s*([^\]]+)\s*\]\s*$")
        ...
        def _major_from_school_tail_if_needed(school: str, major: str) -> tuple[str, str]:
            ...
            - major가 비어 있고, school이 2토큰 이상이며, 마지막 토큰이 학과/전공처럼 보이면 분리.
            """
            if _norm_spaces(major):
                return school, major
            sc = _norm_spaces(school)
            parts = [p for p in sc.split(" ") if p]
            if len(parts) < 2:
                return school, major
            tail = parts[-1]
            # 학과/전공 꼬리 휴리스틱(하드코딩 최소화)
            if any(tail.endswith(suf) for suf in ["학과", "전공", "학부", "과"]) and len(tail) <= 16:
                return _norm_spaces(" ".join(parts[:-1])), tail
            return school, major
            ...
            try:
                ...
                for dt, items in by_date.items():
                    if len(items) == 1:
                        ...
                        continue
                    # 기관 후보: 날짜가 아닌 가장 긴 텍스트
                    inst_best = ""
                    typ_best = ""
                    for it in items:
                        inst = str(it.get("수여기관") or "").strip()
                        typ = str(it.get("종류및근거") or "").strip()
                        if inst and (not _DATE_CELL.match(inst)) and len(inst) > len(inst_best):
                            inst_best = inst
